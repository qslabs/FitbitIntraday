        	...
        	try {
         		// [wwu]
         	 	if (dataType == DataTypes.interday) {
            		...
            		var result = UrlFetchApp.fetch("http://api.fitbit.com/1/user/-/" + currentActivity + "/date/" + dateString+ "/" + dateString + ".json", options);
          		}
        	} catch(exception) {
...
function getAwakeTime(dateString) {
  ...
  for (var i=0; i<2; i++) {
    result = UrlFetchApp.fetch("http://api.fitbit.com/1/user/-/sleep/date/" + dateString + ".json", options); 
    var o = Utilities.jsonParse(result.getContentText());
    sleepLogs = o["sleep"];
